...
const FeaturedCard: React.FC<FeaturedCardProps> = ({ property }) => {
  ...
  const badge = property.isExclusive
    ? t('card.exclusive')
    : property.isNew
      ? t('card.new')
        ...
        <div className="p-6 relative">
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-nordic/5">
            <div className="flex items-center gap-2 text-nordic/60 text-xs font-light">
              <span className="material-icons text-lg text-mosque/40">king_bed</span>
              {property.beds} {t('card.beds')}
            </div>
            <div className="flex items-center gap-2 text-nordic/60 text-xs font-light">
              <span className="material-icons text-lg text-mosque/40">bathtub</span>
              {property.baths} {t('card.baths')}
            </div>
            <div className="flex items-center gap-2 text-nordic/60 text-xs font-light">
              <span className="material-icons text-lg text-mosque/40">square_foot</span>
              {property.sqm} m²
            </div>
          </div>
        </div>
