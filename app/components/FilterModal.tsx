...
const FilterModal: React.FC<FilterModalProps> = ({ isOpen, onClose }) => {
  ...
  const amenities = [
    { name: "Piscina", icon: "pool", active: true },
    { name: "Gimnasio", icon: "fitness_center", active: false },
    { name: "Parqueadero", icon: "local_parking", active: false },
    { name: "Aire Acondicionado", icon: "ac_unit", active: false },
    { name: "Wifi Alta Velocidad", icon: "wifi", active: true },
    { name: "Patio / Terraza", icon: "deck", active: false }
  ];

  const [selectedAmenities, setSelectedAmenities] = useState<number[]>([0, 4]); // Init with pool & wifi
    ...
    >
      ...
      <main className={`relative z-20 w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-transform duration-300 ${isOpen ? 'scale-100' : 'scale-95'}`}>
        ...
        <header className="px-8 py-6 border-b border-black/5 flex justify-between items-center bg-white sticky top-0 z-30">
          <h1 className="text-2xl font-semibold tracking-tight text-nordic">Filtros</h1>
        ...
        <div className="flex-1 overflow-y-auto hide-scroll p-8 space-y-10">
          {/* Section 1: Location */}
          <section>
            <label className="block text-xs font-semibold text-nordic/60 uppercase tracking-wider mb-3">Ubicación o Búsqueda</label>
            <div className="relative group">
              ...
              <input 
                ...
                placeholder="Ciudad, vecindario, dirección o tipo" 
                type="text" 
                value={query}
          ...
          <section>
            <div className="flex justify-between items-end mb-4">
              <label className="block text-xs font-semibold text-nordic/60 uppercase tracking-wider">Rango de Precio</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-clear-day p-3 rounded-lg border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-nordic/60 uppercase font-medium mb-1">Precio Min</label>
              ...
              <div className="bg-clear-day p-3 rounded-lg border border-transparent focus-within:border-mosque/30 transition-colors">
                <label className="block text-[10px] text-nordic/60 uppercase font-medium mb-1">Precio Max</label>
                <div className="flex items-center">
                  <span className="text-nordic/40 mr-1">$</span>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <label className="block text-xs font-semibold text-nordic/60 uppercase tracking-wider">Tipo de Propiedad</label>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-nordic">Habitaciones</span>
              ...
              {/* Baths */}
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-nordic">Baños</span>
                <div className="flex items-center space-x-3 bg-clear-day rounded-full p-1">
                  <button 
          ...
          <section>
            <label className="block text-xs font-semibold text-nordic/60 uppercase tracking-wider mb-4">Comodidades y Características</label>
